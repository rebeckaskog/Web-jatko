  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        ...
        showFormMessage("error", "The name contains characters that are not allowed. Please use only letters.");
        return;
      }
      ...
      if (response.status === 409) {
        ...
        showFormMessage("info", "A resource with the same name already exists, please choose another name.");
        return;
      }
    ...
    showFormMessage("success", "We have successfully received your booking!");
