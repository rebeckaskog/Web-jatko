...
const resourceValidators = [
  ...
  body('resourceName')
    ...
    .trim()            // escape removed
    .matches(/^[a-zA-Z0-9äöåÄÖÅ ]+$/)     //   what is allowed added
    .isLength({ min:5, max: 30 }).withMessage('resourceName must be 5-30 characters'),  // name lenght added
  ...
  body('resourcePriceUnit')
    ...
    .trim()
    .isIn(['hour', 'day', 'week', 'month'])            // week and month added
...
app.post('/api/resources', resourceValidators, async (req, res) => {
  ...
  let {
    ...
    resourceName = '',
    resourceDescription = '',
    resourceAvailable = '',      // false removed
    resourcePrice = 0,
    resourcePriceUnit = ''
  } = req.body;
  ...
  try {
    ...
    `;
    const params = [
      resourceName,      // hex removed
      resourceDescription,
      resourceAvailable,
      resourcePrice,      // *2 removed
      resourcePriceUnit
    ];
